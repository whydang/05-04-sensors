...
public class DrawingSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    /****
     ** Encapsulate touch tracking, thread-safe
     ****/

    public synchronized void addTouch(int id, float x, float y) {
       touches.put(id, new Ball(x, y, 80));
    }
    ...
    public synchronized void moveTouch(int id, float nx, float ny){
        Ball touch = touches.get(id);
        touch.cx = nx;
        touch.cy = ny;
    }
